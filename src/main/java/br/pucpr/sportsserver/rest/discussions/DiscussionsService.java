...
public class DiscussionsService {
    ...
    private Reply reqToReply(String text, Long discussionId, User user) {
        return new Reply(
                discussionsRepository.findById(discussionId)
    ...
    public ReplyResponse addReply(Long id, Long discussionId, String text) {
        var user = usersRepository.findById(id)
                .orElseThrow(() -> new NotFoundException("Id \"" + id + "\" Not Found"));
        var reply = reqToReply(text, discussionId, user);
    ...
    public void deleteDiscussion(Long id, Long discussionId) {
        ...
        discussion.getReplies().stream().toList().forEach(r -> deleteReply(id, r.getId()));
    ...
    public void deleteReply(Long id, Long replyId) {
        ...
        if (reply.getUser().getId() != id && !user.getRoles().contains("ADMIN") && reply.getDiscussion().getUser().getId() != id)
